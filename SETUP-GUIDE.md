...
| 16 | [How to install it as an app (PWA)](#16-how-to-install-it-as-an-app-pwa) | Adding the app to your phone's home screen |
...
## 16. How to install it as an app (PWA)

The app is a **PWA (Progressive Web App)**, so you can install it straight from the browser — no app store, no APK, no Android development tools. Once installed it gets its own home-screen icon and opens full-screen, just like a native app.
...
4. Confirm — the icon appears in your app drawer / home screen
...
> 💡 That's it. The installed app updates itself automatically whenever you push changes (the service worker fetches the new version in the background).

> 📝 **History note:** an earlier version of this project was also packaged as an Android app using a Trusted Web Activity (TWA). That approach is no longer used — the app is now a pure PWA — so the old TWA / `assetlinks.json` setup has been removed.
...
| **Vercel** | The hosting service where your app lives |
